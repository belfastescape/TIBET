...
import { ChevronDown, Clock, Users, Star, ArrowRight, CheckCircle2, MapPin, PartyPopper, Globe, DollarSign, Smile, UsersRound } from "lucide-react"
...
export default function HomeA() {
  ...
  return (
    <div className="min-h-screen">
      ...
      {/* Icon stats — same style as Operation Pitt game-stats */}
      <section className="py-12 bg-[#0c0c0c]">
        <div className="container mx-auto px-4">
          ...
          >
            {[
              { icon: <Smile className="h-20 w-20" />, value: "5/5", label: "FUN" },
              { icon: <MapPin className="h-20 w-20" />, value: "5/5", label: "LOCATION" },
              { icon: <DollarSign className="h-20 w-20" />, value: "5/5", label: "VALUE" },
              { icon: <Users className="h-20 w-20" />, value: "2-8", label: "Players", subLabel: "Per Game" },
              { icon: <UsersRound className="h-20 w-20" />, value: "Up to 28ppl.", label: "GROUPS" },
              { icon: <Clock className="h-20 w-20" />, value: "60 mins", label: "Duration" },
            ].map((item, i) => (
              <div
                key={i}
                className="group flex flex-col items-center gap-4 py-8 px-6 text-center transition-all duration-300 ease-out hover:-translate-y-1"
              >
                <div className="relative w-[175px] h-[175px] transition-transform duration-300 ease-out group-hover:scale-110">
                  <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[112px] h-[112px] rounded-full bg-white/70 opacity-50 group-hover:opacity-100 transition-opacity duration-300 blur-xl z-0" />
                  <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[110px] h-[110px] rotate-45 border border-white bg-black z-10" />
                  <span className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-20 text-white">
                    {item.icon}
                  </span>
                </div>
                <div>
                  {item.value && <p className="text-4xl font-bold text-white font-mono">{item.value}</p>}
                  {item.label && <p className={`text-base text-white mt-1 ${item.subLabel ? "font-bold" : ""}`}>{item.label}</p>}
                  {item.subLabel && <p className="text-sm mt-1 text-white">{item.subLabel}</p>}
                </div>
              </div>
            ))}
          </motion.div>
      ...
      <section id="escape-rooms" className="bg-gray-900 py-20">
        {/* Section heading */}
